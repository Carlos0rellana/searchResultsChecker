...
const checkItem =async (item:[string,string]): Promise <articleModifly|null> => {
    if(item[0]?.length>3 && item[1]?.length>3){
        ...
        if(tempConfig?.subtype!==undefined && tempConfig.subtype){
...
export const readyToUpdateSingleArticle = async (siteUrl:string,values:modValues): Promise<articleModifly|null> => {
    ...
    if(findData!==null && findData.data._id!== undefined){
        let tempData: articleModifly = {
            site: findData.site,
            path: findData.path,
...
export const readyToUpdateListArticle = async (sheetId: string): Promise<[articleModifly,number][]|null> => {
    try {
      const urlList: [articleModifly,number][] = []
      let rows = await accessToGoogleSheets(sheetId,'UrlList')
      if(rows !== null){
...
export const searchAndUpdateArticlesBucle =async (sheetId:string):Promise <[articleModifly,number][] | null> => {
    try{
        const tempList = await readyToUpdateListArticle(sheetId)
        const outputName = 'output'
        let checkOutput = await checkSpreadSheet(sheetId,outputName)
        if(tempList!==null){
            ...
            for(let e = 0 ; e < tempList.length ; e++){
                let element = tempList[e]
                let toSave = element[0]    
